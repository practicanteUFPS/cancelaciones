...
class TCPDF {
	...
	public function __construct($orientation='P', $unit='mm', $format='LETTER', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->default_form_prop = array('lineWidth'=>1, 'borderStyle'=>'solid', 'fillColor'=>array(255, 255, 255), 'strokeColor'=>array(0, 0, 0));
		...
		if ($this->header_xobjid === false) {
			...
			if ($this->rtl) {
				...
				$this->SetX($this->original_lMargin);
                        ...
                        $hoy = date_format($date_hoy, 'd/m/Y');                        
                        $this->Cell(($this->w - $this->original_lMargin - $this->original_rMargin), 0, 'Generado: ' . $hoy, 'T', 1, 'R');
//                      LINEA RECTA
//			$this->Cell(($this->w - $this->original_lMargin - $this->original_rMargin), 0, '', 'T', 0, 'C');
	...
	public function Footer() {
		...
		if ($this->getRTL()) {
			$this->SetX($this->original_rMargin);
			$this->Cell(0, 0, "pag " . $pagenumtxt, 'T', 0, 'L');
		} else {
			$this->SetX($this->original_lMargin);
			$this->Cell(0, 0, "pag " . $pagenumtxt, 'T', 0, 'C');
		}
	}
