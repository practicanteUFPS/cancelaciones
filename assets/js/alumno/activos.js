$(document).ready(function () {
    ...
    $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
        ...
        var sexo = data[3];
        var aprobados = parseFloat(data[4]) || 0;
        var cursados = parseFloat(data[5]) || 0;
    ...
    radios.forEach(function (radio) {
        radio.addEventListener('change', function () {
            ...
            if (radio.value === 'semestre') {
                labelUnico.textContent = 'activos del semestre:';
                campoUnico.classList.remove('hidden');
            } else if (radio.value === 'desde') {
                labelUnico.textContent = 'activos desde semestre:';
                campoUnico.classList.remove('hidden');
...
async function exportarPDF(dt) {
    ...
    doc.setFontSize(14);
    doc.text("Alumnos activos", 50, 20);
